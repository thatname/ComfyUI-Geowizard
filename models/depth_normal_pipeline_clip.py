...
class DepthNormalEstimationPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        _, height, width = input_image.size()
        ...
        rgb_norm = input_image * 2.0 - 1.0
        rgb_norm = torch.clamp(rgb_norm, min=-1.0, max=1.0)
        rgb_norm = rgb_norm.to(dtype=self.dtype, device=device)
        ...
        pbar_1 = comfy.utils.ProgressBar(len(single_rgb_dataset))
        ...
        depth_pred_tensor = (depth_pred - min_d) / (max_d - min_d)
        ...
        return DepthNormalPipelineOutput(
            depth_np = depth_pred,
            depth_pred_tensor = depth_pred_tensor,
